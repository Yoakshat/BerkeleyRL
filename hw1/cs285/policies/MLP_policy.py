...
class MLPPolicySL(BasePolicy, nn.Module, metaclass=abc.ABCMeta):
    ...
    # ac_dim represents the # of different actions (all taken at once)
    def __init__(self,
                 ac_dim,
                 ob_dim,
                 n_layers,
    ...
    def update(self, observations, actions):
        ...
        """
        assert observations.shape[0] == actions.shape[0], "mismatch in obs and action batch size"
        
        actDistrib = self.forward(observations)

        # maximize log-likelihood of experts actions
        # [batch_size, ac_dim]
        # or minimize NLL
        # what's the log probability under this distribution
        loss = -1 * actDistrib.log_prob(actions).sum(dim=-1).mean()
