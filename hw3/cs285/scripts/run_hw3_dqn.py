...
def run_training_loop(config: dict, logger: Logger, args: argparse.Namespace):
    ...
    # after learning starts, every step is an update
    for step in tqdm.trange(config["total_steps"], dynamic_ncols=True):
        ...
        if isinstance(replay_buffer, MemoryEfficientReplayBuffer):
            ...
            # do we insert all frames one by one
        ...
        if step >= config["learning_starts"]:
            ...
            # DEBUGGING
            # scalar, name, step
            # logger.log_scalar(update_info["q_vs_rtg"], "q_vs_rtg", step)
            logger.log_scalar(batch["actions"].float().mean().item(), "AvgAction", step)
        ...
        if step % args.eval_interval == 0:
            ...
            print("EVAL RETURN: " + str(np.mean(returns))) 
            print("EVAL STD: " + str(np.std(returns))) 
